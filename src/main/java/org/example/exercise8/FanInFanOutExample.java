...
public class FanInFanOutExample {
	...
	static class FanOutExample {
		...
		public CompletableFuture<Void> processItems(List<String> items) {
			...
			List<CompletableFuture<Void>> futures = items
					.stream()
	...
	static class CompleteFanPatternExample {
		...
		public CompletableFuture<Map<String, Double>> analyzeData(List<String> dataSources) {
			try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
				...
				List<CompletableFuture<Map.Entry<String, Double>>> analysisFutures = dataSources
						.stream()
						.map(source -> CompletableFuture.supplyAsync(
								() -> Map.entry(source, analyzeDataSource(source)),
								executor))
						.toList();
	...
	public static void main(String[] args) throws Exception {
		LoggerUtils.info("FanInFanOutExample", "main", "=== FAN-OUT Example ===");
		...
		LoggerUtils.info("FanInFanOutExample", "main", "=== FAN-IN Example ===");
		FanInExample fanIn = new FanInExample();
		List<Integer> aggregated = fanIn.aggregateResults().get();
		LoggerUtils.info("FanInFanOutExample", "main", "Resultado agregado: " + aggregated);
		fanIn.shutdown();

		LoggerUtils.info("FanInFanOutExample", "main", "\n=== Complete Fan Pattern with Virtual Threads ===");
		CompleteFanPatternExample completePattern = new CompleteFanPatternExample();
		Map<String, Double> results = completePattern
				.analyzeData(Arrays.asList("DB1", "API1", "Cache1", "File1", "Queue1"))
				.get();
		results.forEach((source, value) -> LoggerUtils.info(
				"FanInFanOutExample",
				"main",
				source + ": " + String.format("%.2f", value)));
	}
}